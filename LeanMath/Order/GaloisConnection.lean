...
-- this is marked `implicit_reducible` because we use gc to build complete lattices for algebraic substructures
-- based off of `Set`, and for those we typically define `Top` beforehand
-- this will be defeq to `Top` intances generated by `giGenerate`,
-- but lean won't see that unless we mark this as `implicit_reducible`.
@[implicit_reducible]
