...
@[implicit_reducible]
private def instSigma' (card: Fin n -> ℕ) : FinEncodable (Σi, Fin (card i)) where
  card := ∑i, card i
...
variable {ι: Sort*} {α: ι -> Sort*} [ft: FinEncodable ι] [fα: ∀i, FinEncodable (α i)]

private def factor
  (rι: (Fin (FinEncodable.card ι) ↭ ι) ⊕' (Fin (FinEncodable.card ι) ≃ ι))
  (rα: (∀i, Fin (FinEncodable.card (α (bijOrEquiv rι i))) ↭ α (bijOrEquiv rι i)) ⊕' (∀i, Fin (FinEncodable.card (α (bijOrEquiv rι i))) ≃ α (bijOrEquiv rι i)))
: (
    Σ' (rι': Fin (FinEncodable.card ι) ↭ ι), ∀i, Fin (FinEncodable.card <| α (rι' i)) ↭ α (rι' i)
  ) ⊕' (
    Σ' (rι': Fin (FinEncodable.card ι) ≃ ι), ∀i, Fin (FinEncodable.card <| α (rι' i)) ≃ α (rι' i)
  ) := match rι, rα with
...
instance : FinEncodable (Σ'i, α i) := by
  let rι := bij_or_eqv ι
  let rα := fun i => bij_or_eqv (α (bijOrEquiv rι i))
  let : Fintype ι := inferInstance
  let (i: ι) : Fintype (α i)  := inferInstance
  have := factor (ι := ι) (α := α) rι (factorBijOrEquiv rα)
  match this with
  | .inl ⟨rι, rα⟩ =>
    have := @instSigma'
...
variable {ι: Type*} {α: ι -> Type*} [ft: FinEncodable ι] [fα: ∀i, FinEncodable (α i)]

instance : FinEncodable (Σi, α i) :=
